export default function RateLimitingPage() {
  ...
  "action": "block"           // Action when limit is exceeded
}`}</code>
          </pre>
...
const rate = ZSecure({
    ...
    baseUrl: "YOUR_BASE_URL||http://localhost:3000", // Base URL for the ZSecure service
    rateLimitingRule : {
        rule : {
...
`}</code>
  ...
  "identifier": "ip",         // How to identify clients
  "action": "throttle"        // Action when token bucket is empty
}`}</code>
          </pre>
...
`}</code>
  ...
  "identifier": "ip",         // How to identify clients
  "action": "discard"         // Action when bucket overflows
}`}</code>
          </pre>
